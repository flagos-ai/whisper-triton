...
def measure_rtf(model, audio_path: str, audio_duration_s: float, fp16: bool) -> dict:
    ...
    for _ in range(WARMUP_RUNS):
        _ = whisper.transcribe(
            model, audio_path, language="en", temperature=0.0, fp16=fp16
        )
    ...
    throughput_audio_hrs = (audio_duration_s / 3600) / (
        avg_time / 3600
    )  # audio_hrs / gpu_hr
...
def run_benchmark(models: list, durations: list) -> dict:
    ...
    for model_name in models:
        print(f"\n>>> Model: {model_name}")
        print(
            f"  {'Precision':<8} {'Duration':>9} {'Infer(s)':>9} {'RTF':>7} {'Throughput(x)':>14} {'GPU Mem(MiB)':>13}"
        )
        print("  " + "-" * 70)
        ...
        for precision in ["fp32", "fp16"]:
            ...
            for dur in durations:
                ...
                if not os.path.exists(audio_path):
                    print(
                        f"  {precision:<8} {dur:>6}s    SKIP (file not found: {audio_path})"
                    )
                    done += 1
                    continue
                ...
                try:
                    ...
                    results.append(row)
                    print(
                        f"  {precision:<8} {dur:>6}s  {stats['avg_infer_time_s']:>9.2f}"
...
def print_summary(data: dict):
    ...
    print(
        f"GPU: {data['gpu_name']}  |  Averaged over {data['bench_runs']} runs (after {data['warmup_runs']} warmup)"
    )
    print("=" * 100)
    ...
    for model_name in models:
        print(f"\n  Model: {model_name}")
        print(
            f"  {'Precision':<8} {'Audio':>7} {'RTF':>8} {'Throughput(x)':>14} {'Mem(MiB)':>10}  FP16 speedup"
        )
        print("  " + "-" * 65)

        model_rows = {
            (r["precision"], r["audio_duration_s"]): r
            for r in results
            if r["model"] == model_name and "error" not in r
        }

        durations = sorted(
            set(r["audio_duration_s"] for r in results if r["model"] == model_name)
        )
        for dur in durations:
            ...
            if fp32:
                ...
                print(
                    f"  {'fp32':<8} {dur:>4}s  {fp32['rtf']:>8.4f}  {fp32['throughput_audio_hrs_per_gpu_hr']:>14.2f}"
                    f"  {fp32['peak_gpu_mem_mib']:>10.1f}  {speedup}"
                )
            if fp16:
                print(
                    f"  {'fp16':<8} {dur:>4}s  {fp16['rtf']:>8.4f}  {fp16['throughput_audio_hrs_per_gpu_hr']:>14.2f}"
